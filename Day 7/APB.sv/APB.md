...
<img width="1860" height="678" alt="image" src="https://github.com/user-attachments/assets/b16ac0cb-4b6c-40fd-97bd-a0203264848b" />
Figure 1: Timing diagram trace for standard APB bus executions (apb_waveform.png)

Subcomponent Descriptions
apb_transaction (Data Packet): Encapsulates randomize-ready bus contents. It includes custom address constraints to force 32-bit word alignment (addr[1:0] == 2'b00) spanning from 0x0000_0000 to 0x0000_03FC.
generator (Stimulus Source): Procedurally handles loop tracking to feed exactly 10 sequence pairs of randomized write-then-read transactions into the pipeline.
driver (Pin Master): Unpacks incoming transaction requests from the generator mailbox and physically drives the corresponding interface pins.
monitor (Passive Sniffer): Samples active wires at each clock boundary and ships reconstructed transaction logs over to checking components via mailboxes.
scoreboard (Golden Model Checker): Employs a SystemVerilog associative array structure (internal_mem) to store historical write states and dynamically evaluate the accuracy of live read operations.
